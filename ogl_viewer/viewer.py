...
class UserAction:
    def __init__(self):
        ...
        self.decrease_height = False
        self.increase_normal = False
        self.decrease_normal = False
        self.hit_coord = []
...
class GLViewer:
    ...
    def init(self, _params, _has_imu): 
        ...
        glutCreateWindow("ZED Floor Detection")
    ...
    def update_view(self, _image, _pose, _tracking_state, _height_threshold, _normal_threshold):     
        self.mutex.acquire()
        if self.available:
            # update image
            ...
            self.height_threshold = _height_threshold
            self.normal_threshold = _normal_threshold
    ...
    def keyReleasedCallback(self, key, x, y):
        if ord(key) == 27:   # Esc
            self.close_func()
        if ord(key) == ord('a'):
            self.user_action.decrease_height = True
        if ord(key) == ord('d'):
            self.user_action.increase_height = True
        if ord(key) == ord('q'):
            self.user_action.decrease_normal = True
        if ord(key) == ord('e'):
            self.user_action.increase_normal = True
    ...
    def print_text(self):
        if self.available:
            ...
            self.print_GL(-0.99, y, "Press 'q' or 'e' to increase/decrease the normal threshold")
            y -= 0.05
            self.print_GL(-0.99, y, format(self.normal_threshold, '.2f'))
            y -= 0.05
            self.print_GL(-0.99, y, "Press 'a' or 'd' to increase/decrease the height threshold")
            y -= 0.05
            self.print_GL(-0.99, y, format(self.height_threshold, '.2f'))
